...
# 내부 후속 조사의 결론을 ``연구는 … 확정한다 — 실제 사실``처럼 쓴 경우다.
# 대시 뒤의 사실은 보존하되, 독자 화면에서 조사 주체를 주어로 내세우지 않는다.
# 학술 연구 자체가 시장 사건인 평서문은 대시가 없으므로 이 규칙에 걸리지 않는다.
RESEARCH_PROCESS_ASSERTION_CORE = (
    r"(?:(?:이번|심층|추가)\s*)?(?:연구|조사)(?:에서는?|에서|는|가)\s*"
    r"[^.!?。！？—–]{0,240}(?:확정(?:한다|했다)|확인(?:한다|했다)|"
    r"보여준다|드러낸다)\s*[—–]\s*"
)
READER_PROCESS_BOILERPLATE_RE = re.compile(
    r"(?:추가\s*연구(?:\s*(?:근거|결과|제한))?\s*[:：]?|"
    r"(?<![0-9A-Za-z가-힣])" + RESEARCH_PROCESS_NARRATION_CORE + r"|"
    r"(?<![0-9A-Za-z가-힣])" + RESEARCH_PROCESS_ASSERTION_CORE + r"|"
