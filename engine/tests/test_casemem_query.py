...
def test_query_matches_2018_inventory_phase(tmp_path):
    s = _seeded(tmp_path)
    res = query_case_memory(s, signals=["고객 재고조정 시작", "재고 증가"],
                            as_of="2018-10-01", sector="memory")
    assert res.sector == "memory"
    ids = {m.episode_id for m in res.matches}
    assert "mem-2016-2019-supercycle-crash" in ids


def test_query_blocks_future_phase_leakage(tmp_path):
    # 수요붕괴/CapEx삭감 국면(knowable 2018-12-18/2019-06-25)은 2018-03-01엔 미가시
    s = _seeded(tmp_path)
    res = query_case_memory(s, signals=["수요 약화 재고조정 CapEx 삭감"],
                            as_of="2018-03-01", sector="memory")
    for m in res.matches:
        if m.episode_id == "mem-2016-2019-supercycle-crash":
            assert m.matched_phase_order < 2       # 국면2~4(2018-09 이후 knowable)면 누출
...
def test_query_without_llm_is_deterministic(tmp_path):
    s = _seeded(tmp_path)
    res = query_case_memory(s, signals=["고객 재고조정"], as_of="2018-10-01",
                            sector="memory")
