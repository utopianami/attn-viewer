...
export function scorePlaybook(verdicts) {
  ...
  // orderScore: fraction of (i<j) pairs (by actual order index) where predictedOrder_i < predictedOrder_j
  const coveredItems = verdicts
    .map((v, i) => ({ actualIdx: i, predictedOrder: v.predictedOrder }))
    .filter((v) => v.predictedOrder !== null);
  let orderScore = 1;
  if (coveredItems.length >= 2) {
    let pairs = 0, ordered = 0;
    for (let i = 0; i < coveredItems.length; i++) {
      for (let j = i + 1; j < coveredItems.length; j++) {
        pairs++;
        if (coveredItems[i].predictedOrder < coveredItems[j].predictedOrder) ordered++;
      }
    }
    orderScore = pairs ? ordered / pairs : 1;
  }
  return {
