...
export async function saveRegistry(corpusRoot, registry) {
  ...
  await writeFile(registryPath(corpusRoot), `${JSON.stringify(registry, null, 2)}\n`, "utf8");
}

export async function upsertBlog(corpusRoot, { id, name, source = "naver" }) {
...
export async function blogStats(corpusRoot, blog) {
  ...
  for (const row of rows) {
    if (!latest || logNoOf(row) > logNoOf(latest)) {
      latest = row;
    }
  }
  return {
...
export async function listPosts(corpusRoot, { blogIds, offset = 0, limit = 50 }) {
  const rows = [];
  ...
  rows.sort((first, second) => logNoOf(second) - logNoOf(first));
  return {
    total: rows.length,
    posts: rows.slice(offset, offset + limit),
  };
}
