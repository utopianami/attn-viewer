...
// 인용문을 마침표+공백 경계로 문장 단위 분할하여 각각이 body에 있는지 확인한다.
// 개조식 원문에 목록 번호("4.")·이미지 플레이스홀더("![...]")가 끼어도 문장 각각은 원문과 일치하므로 통과.
function matchesInFragments(quote, haystack) {
  const fragments = normalize(quote).split(/(?<=\.)\s+/).filter((f) => f.length >= 5);
  if (fragments.length === 0) return false; // 모두 5자 미만 → 실패
