...
// 인용문을 문단(줄바꿈) 경계 → 문장 종결 부호+공백 경계 순으로 분할하여
// 각 조각이 body에 순서대로 있는지 확인한다.
// 개조식 원문에 목록 번호("4.")·이미지 플레이스홀더("![...]")가 끼어도 문장 각각은 원문과 일치하므로 통과.
// 문단 분할이 먼저인 이유: "~같음"·"~보면"처럼 종결 부호 없이 끝나는 문장 뒤에 이미지가 끼면
// 문장부호 분할만으로는 앞뒤 문단이 한 조각으로 붙어 오탐이 난다 (tosoha1 실측, 2026-07-14).
...
function matchesInFragments(quote, haystack) {
  const fragments = String(quote)
    .split(/\n+/)
    .flatMap((p) => normalize(p).split(/(?<=[.!?…])\s+/))
    .filter((f) => f && !MARKER_RE.test(f));
