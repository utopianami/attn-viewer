...
import { chmod, mkdtemp, mkdir, writeFile, rm, readFile, readdir } from "node:fs/promises";
...
test("runCodexSummary removes output-last-message after success", async () => {
  const fakeBin = await mkdtemp(join(tmpdir(), "fake-codex-bin-"));
  const fakeTmp = await mkdtemp(join(tmpdir(), "summary-output-"));
...
process.stdin.on("end", () => fs.writeFileSync(process.argv[index + 1], "summary result\\n"));
`,
    "utf8",
  );
  ...
  try {
    assert.equal(await runCodexSummary("prompt", { timeoutMs: 5_000 }), "summary result");
    assert.deepEqual(await readdir(fakeTmp), []);
  } finally {
