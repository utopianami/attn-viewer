...
import { savePlaybook, loadPlaybooks, validatePlaybook, loadAllCards, loadAllCardsWithReview, buildClusterPrompt, pickHoldout, buildPlaybookPrompt, synthesizePlaybook, buildPredictionPrompt, buildJudgePrompt, scorePlaybook, alignVerdicts } from "./playbooks.mjs";
...
test('alignVerdicts: order 정렬, 누락 항목은 covered=false, hasKill은 카드에서 결정, 중복은 첫 번째', () => {
  ...
  const items = [
    { order: 1, covered: true, killCovered: true, hasKill: false },   // hasKill 무시 — 코드가 결정
    { order: 1, covered: false, killCovered: false, hasKill: true },  // 중복 order 1 — 무시
    { order: 2, covered: false, killCovered: false, hasKill: true },  // hasKill 무시
    // order 3 없음 — 누락 처리
  ];
  const result = alignVerdicts(card, items);
  assert.equal(result.length, 3);
  assert.deepEqual(result[0], { covered: true, killCovered: true, hasKill: true });   // order 1, kill있음
  assert.deepEqual(result[1], { covered: false, killCovered: false, hasKill: false }); // order 2, kill없음
  assert.deepEqual(result[2], { covered: false, killCovered: false, hasKill: true }); // order 3, 누락→false, kill있음
});
