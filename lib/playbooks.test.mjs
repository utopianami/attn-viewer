...
test("validatePlaybook: 구조 게이트끼리 order가 중복되면 all-or-none으로 문자열 게이트 강등", () => {
  // 3부 T11 블로커3(c) — 엔진 소비측(engine/stages/playbook.py parse_gate_checks)과
  // 대칭: 같은 order를 공유하는 구조 게이트는 전부 구조 해석에서 드롭돼야 소비측
  // order 룩업이 잘못된 게이트에 판정을 붙이는 사고(codex 최종 리뷰 재현)를 막는다.
  const base = { order: 1, check: "c", operationalization: "o", evidence: ["k1"] };
  const full = { ...base, metric_id: "memory_price_usd_per_gb",
    selector: { meta_filter: { category: "DRAM" } }, aggregation: "last",
    comparator: ">=", threshold: 0.05, unit: "USD/GB", max_age_days: 45 };
  const dupSameOrder = { ...full, check: "c2", metric_id: "search_interest_kr", unit: "index" };
  const { playbook, dropped } = validatePlaybook(
    { gates: [full, dupSameOrder] }, new Set(["k1"]));
  assert.equal(playbook.gates[0].metric_id, undefined);   // 중복 order → 강등
  assert.equal(playbook.gates[1].metric_id, undefined);   // 중복 order → 강등
  assert.ok(dropped.some((d) => d.includes("중복 order")));
  // 서로 다른 order면 정상 보존
  const other = { ...full, order: 2, check: "c3" };
  const kept = validatePlaybook({ gates: [full, other] }, new Set(["k1"]));
  assert.equal(kept.playbook.gates[0].metric_id, "memory_price_usd_per_gb");
  assert.equal(kept.playbook.gates[1].metric_id, "memory_price_usd_per_gb");
});
