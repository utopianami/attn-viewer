...
[스케줄러: 하루 2회]
  ...
  6. 가격 스냅샷: storage/rag/memory_sector/prices/YYYY-MM-DD.json
```
...
├─ 오늘의 신호 요약 (sonnet 3~5줄, 카드 근거 링크)
├─ 이벤트 타임라인 (세로, 최신순)
...
1. **P1 수집기**: 쿼리 매트릭스 + 판정 + 카드 저장 + 스케줄러. 산출물: index.jsonl 쌓임
...
1. D램/낸드 가격 데이터 — 뉴스 간접 수집으로 충분? 유료 소스 붙일지?
2. 스케줄러 위치 — engine 내 APScheduler vs 시스템 cron vs node(server.mjs) 쪽?
3. B/C 엔티티 확정 — 엔비디아를 어디 두나? (메모리 소비자이자 A' 성격도 있음 → 별도 축?)
4. 대시보드 진입점 — 기존 index.html 내 탭 vs 별도 페이지?
5. codex와 분업 경계 제안: **claude = engine(P1 수집·판정·검색 API), codex = UI(P2·P3 렌더)** — 역제안 환영
