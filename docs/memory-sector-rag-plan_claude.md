...
### 1-2. 확장 축 (2026-07-06 검증 라운드 추가 — 사이클 판정에 필수)

| 축 | 구성 | 왜 필요한가 |
|---|---|---|
| **E 전통 최종수요** | 스마트폰·PC·일반서버·자동차 (IDC/카운터포인트 출하 보도자료, 애플·레노버 실적, 중국 보조금 뉴스) | **범용 D램·낸드의 절반은 여전히 폰·PC가 결정** (2023 다운사이클의 원인). E축 없이는 "HBM 타이트 + 범용 과잉" 이중 사이클을 못 읽음 |
| **P 정책/지정학** (충격 노드) | 수출통제(HBM·장비 대중 규제), 관세, 보조금, **CXMT 증산 등 중국발 공급 교란** | 수요-공급 사슬 밖에서 임의 엣지를 때리는 외생 충격. 카드에 axis=P 태깅. A축 감시 엔티티에 CXMT·키옥시아 추가 |
| **M 거시/유동성** (배경 레이어) | 금리(US10Y)·원달러 환율·유동성 (yahoo macro 기수집) | 특정 엣지가 아니라 전 축 동시 작용. 원달러는 삼전·하이닉스 실적 직접 효과 |
...
① 주가가 이미 반영한 것을 지표로 검증, ② 주가-지표 괴리 탐지 — 괴리는 "과민반응"일
수도 "시장이 먼저 아는 것"일 수도 있으므로 **두 해석을 모두 표시**한다.
...
```json
{
  ...
  "axis": "B",                        // A | A'(공급망/패키징) | B | C | E(전통수요) | P(정책충격)
  "entities": ["META"],
...
- E: `smartphone shipment forecast`, `PC DRAM content`, `중국 스마트폰 보조금`
- P: `HBM export control`, `semiconductor tariff`, `CXMT DRAM capacity`
...
| **반도체 생산·출하·재고지수** ✅ | A | **통계청 KOSIS 오픈API** (광업제조업동향, 월간 — 2026-07-06 검증. 반도체 세부산업 코드 P1 확인) | 월간 | **재고/출하 비율 = 고전적 메모리 사이클 시계.** 재고↓+출하↑ = 업사이클 초입 |
| **D램 수출물가지수** ✅ | A/D | **한국은행 ECOS 오픈API** (수출물가지수 품목별, 월간 — API 실재 검증, D램 품목코드 P1 확인) | 월간 | 삼전·하이닉스 D램 ASP의 월간 공식 프록시 — TrendForce 없이 가격 시계열 확보 |
| **엔비디아 데이터센터 매출** | 경로 중간 | yahoo (분기 실적) | 분기 | C→B→**GPU**→HBM 경로 중간 노드의 실측치 — HBM 수요의 가장 직접적인 공개 숫자 |
| 대만 전 상장사 월매출 ✅ | B/A' | **MOPS 오픈데이터 CSV 직다운** (`mopsfin.twse.com.tw/opendata/t187ap05_L.csv`, YoY 컬럼 포함 — 2026-07-06 실측) | 월간 | ODM·TSMC 필터만 하면 됨. 🟡→✅ 승격 |
| 메모리 장기 가격 ✅ | D | **Stanford DAM** CSV (2026-07-06 접근 검증) | 갱신주기 P1 확인 | DRAM/HBM/NAND 장기 시계열 |
| AI칩 HBM 소비 추정 ✅ | C/A' | **Epoch AI** 데이터 다운로드 (2026-07-06 접근 검증) | 비정기 | HBM 수요 프록시 |
...
**최종 출력: 메모리 사이클 상태 판정** (2026-07-06 추가 — 이 시스템의 존재 이유.
목적이 "업다운 파악"인데 판정 출력이 없었음). 4요소를 합성해 대시보드 최상단에 표시:

```
① 가격  : 현물 방향 + 현물-고정 스프레드 + ECOS D램 수출물가 YoY
② 재고  : KOSIS 재고/출하 비율 방향 + 메모리사 DIO
③ 수요  : 관세청 수출 YoY + ODM 월매출 YoY + 토큰 총지출 방향
④ 공급  : 장비 수주 모멘텀 + capex 급증 (Supply Overbuild Risk)

→ 상태: 업사이클 | 다운사이클 | 전환 의심 (+ 요소별 기여, 근거 카드 링크)
→ 규칙 기반(가중 합산)으로 시작 — LLM 판정 아님. 재현 가능·설명 가능해야 함
```

시각화: **재고/출하 순환도** (KOSIS 2축 4분면 — 재고↓·출하↑=회복기 → 재고↑·출하↑=
확장기 → 재고↑·출하↓=둔화기 → 재고↓·출하↓=수축기). 반도체 사이클 분석의 고전 차트를
자동 데이터로 상시 갱신.
...
**✅ 완전 자동 추가** (전부 2026-07-06 검증): 관세청 10일 수출/수입 잠정치 오픈 API /
SaveTicker 캘린더 API(매크로 이벤트) / **MOPS 월매출 CSV 직다운**(t187ap05_L.csv 실측) /
**Stanford DAM** 가격 CSV / **Epoch AI** 데이터 / **KOSIS** 생산·출하·재고지수 오픈API /
**ECOS** 수출물가지수 오픈API (KOSIS·ECOS는 무료 키 + 품목코드만 P1 확인)

**🟡 자동 가능, P1에서 검증 또는 깨짐 대비**: TrendForce 공개 차트 스크랩
(깨지면 보도자료 경유로 자동 강등), 한경컨센서스 리포트(스크랩 검증), WSTS/SIA 보도자료
...
2. API 키 발급 필요(전부 무료): OpenRouter(데이터셋), data.go.kr(관세청 수출),
   KOSIS(생산·출하·재고지수), ECOS 한국은행(D램 수출물가지수)
